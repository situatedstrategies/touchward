...
export function ColorWheel({ value, onChange, theme }: Props) {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <HueWheel size={WHEEL_SIZE} brightness={hsv.v} />
        <View
          pointerEvents="none"
          style={[
            styles.thumb,
 ...
 * saturation at the edge, drawn as wedges each with its own radial gradient.
 * Brightness darkens the whole wheel so the thumb always shows a true color.
 */
const HueWheel = memo(function HueWheel({
  ...
  return (
    <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      <Circle cx={r} cy={r} r={r} fill="#000000" opacity={1 - brightness} />
    </Svg>
  );
});
