...
- Pulsar haptics (development and store builds): the reward modes play as
  ...
  151 presets. Builds without Pulsar play the built-in patterns instead.
...
Then build and install the dev client on a connected phone with
`npx expo run:ios --device` (or open `ios/Touchward.xcworkspace` in Xcode and
run the Touchward scheme). The installed app connects to Metro from
`npm start` and reloads as you edit.
...
The app detects each native module at runtime and falls back to the built-in
haptics when one is missing.
...
The `ios/` and `android/` folders are generated by prebuild and committed, so
Xcode and Android Studio open them directly.
...
runs every time the app comes to the foreground.
